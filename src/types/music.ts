 ...
 * - 时间 / 大小格式化
 *
 * 位置说明：原在 `src/components/music/types.ts`。服务端 `src/lib/music-client.ts` 需要引用
 * 这批模型，此前只能反向依赖组件目录（服务端层 import 视图层），方向是错的。现上提到
 * `src/types/`，对齐「类型集中」约定，组件层与服务端共用同一份定义。
 */
 ...
 * ⚙️ 这是「完整平台全集」，含 tencent（QQ音乐）——是否出现在搜索面板 /
 ...
 * - migu 无内置直链引擎：搜索结果默认只识别展示，点播时按 GD 直链通道尝试取链。
