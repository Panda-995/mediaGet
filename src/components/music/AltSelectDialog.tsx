...
function noteOf(c: AltCandidate): string | null {
  ...
  if (c.provenance === "cache") {
    // 来源 C：共享缓存里曾真实播放成功过的版本。它们按 auto 处理，正常不会出现在面板；
    // 若因自动尝试预算耗尽而露出，明确标注，避免被误认成「专辑不同」的人工候选
    return "曾成功播放过的版本（本次自动尝试次数已用尽）";
  }
  if (!c.auto) {
 ...
 * 列出同曲其他版本（来源 A：队列内近似；来源 C：共享缓存里曾成功播放过的版本；
 * 来源 B：跨音源现搜到的同名曲），由用户挑一版「就播这版」。
 * 非高置信候选带注释徽标，仅人工确认。
 */
