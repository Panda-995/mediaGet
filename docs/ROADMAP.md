...
**⚠️ 与音乐页收藏的区别**（2026-09 已落地，见 `musicEngine.md` §7.4）：本节指的是**视频解析结果**的收藏（IndexedDB `history`/`favorites` 两个 store + 独立页面）。音乐页 `/music` 已另行实现**曲目收藏**：`src/components/music/favorites.ts` + `favorites-store.ts`，载体是 localStorage（单份 JSON、上限 500、同步可读，适合「渲染期判定是否已收藏」），入口是结果行 / 正在播放卡片 / 底部播放条 / 「我的收藏」视图四处。两者数据形态与消费场景不同（曲目体积小、需 O(1) 判定；视频结果含多分P/图集、只在下钻时读取），**不要为了「统一」把音乐侧迁去 IndexedDB**。
